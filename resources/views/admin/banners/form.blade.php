<div class="form-group {{ $errors->has('banner_name') ? 'has-error' : ''}}">
    ...
    <div class="col-md-6">
        <input class="form-control " name="banner_name" type="text" id="banner_name"
               value="{{ $banner->banner_name or ''}}">
...
<div class="form-group {{ $errors->has('status') ? 'has-error' : ''}}">
    ...
    <div class="col-md-6">
        <input name="status" value=1 type="radio"
               id="status" {{ isset( $banner->status) ? $banner->status == 1 ? 'checked' :'' : '' }}>Active
        <input name="status" value=0 type="radio"
               id="status" {{isset( $banner->status) ? $banner->status == 0 ? 'checked' :'' : '' }}>Inactive
