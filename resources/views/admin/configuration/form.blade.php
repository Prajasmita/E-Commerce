<div class="form-group {{ $errors->has('conf_key') ? 'has-error' : ''}}">
    ...
    <div class="col-md-6">
        <input class="form-control" name="conf_key" type="text" id="conf_key"
               value="{{ $configuration->conf_key or ''}}" {{isset($configuration->conf_key) ? 'readonly' : ''}}>
...
<div class="form-group {{ $errors->has('conf_value') ? 'has-error' : ''}}">
    ...
    <div class="col-md-6">
        <input class="form-control" name="conf_value" type="text" id="conf_value"
               value="{{ $configuration->conf_value or ''}}">
...
<div class="form-group {{ $errors->has('status') ? 'has-error' : ''}}">
    ...
    <div class="col-md-6">
        <input name="status" value="1" type="radio"
               id="status" {{ isset( $configuration->status) ? $configuration->status == 1 ? 'checked' :'' : '' }}>Active
        <input name="status" value="0" type="radio"
               id="status" {{ isset( $configuration->status) ? $configuration->status == 0 ? 'checked' :'' : '' }}>Inactive
