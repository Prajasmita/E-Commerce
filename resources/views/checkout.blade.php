                        ...
                        <div class="col-sm-6 clearfix">
                            <div class="bill-to">
                                ...
                                <div class=" col-sm-12">
                                    ...
                                    <div class="col-sm-6">
                                        <input type="text" name="user_id" id="user_id" value="{{$user_id}}" class="hidden_field">

                                        <div class="form-group {{ $errors->has('company_name') ? 'has-error' : ''}}">
                                                <input type="text" name="company_name" id="company_name" value="{{$user->company_name or ''}}" placeholder="Company Name" class="checkout-form-input">
                                                {!! $errors->first('company_name', '<span class="help-block">:message</span>') !!}
                                        </div>
                                        <div class="form-group {{ $errors->has('email') ? 'has-error' : ''}}">
                                            <input type="text" name="email" value="{{$user->email or ''}}" placeholder="Email*" class="checkout-form-input">
                                            {!! $errors->first('email', '<span class="help-block">:message</span>') !!}
                                        </div>
                                        <div class="form-group {{ $errors->has('title') ? 'has-error' : ''}}">
                                            <input type="text" name="title" value="{{$user->title or ''}}" placeholder="Title" class="checkout-form-input">
                                            {!! $errors->first('title', '<span class="help-block">:message</span>') !!}
                                        </div>
                                        <div class="form-group {{ $errors->has('first_name') ? 'has-error' : ''}}">
                                            <input type="text" name="first_name" value="{{$user->first_name or ''}}" placeholder="First Name *" class="checkout-form-input">
                                            {!! $errors->first('first_name', '<span class="help-block">:message</span>') !!}
                                        </div>
                                        <div class="form-group {{ $errors->has('middle_name') ? 'has-error' : ''}}">
                                            <input type="text" name="middle_name" value="{{$user->middle_name or ''}}" placeholder="Middle Name" class="checkout-form-input">
                                            {!! $errors->first('middle_name', '<span class="help-block">:message</span>') !!}
                                        </div>
                                        <div class="form-group {{ $errors->has('last_name') ? 'has-error' : ''}}">
                                            <input type="text" name="last_name" value="{{$user->last_name or ''}}" placeholder="Last Name *" class="checkout-form-input">
                                            {!! $errors->first('last_name', '<span class="help-block">:message</span>') !!}
                                        </div>
                                        <div class="form-group {{ $errors->has('address1') ? 'has-error' : ''}}">
                                            <input type="text" name="address1" value="{{$user->address1 or ''}}" placeholder="Address 1 *" class="checkout-form-input">
...
--}}
                                        ...
                                        <div class="col-sm-6">
                                            <div class="form-group {{ $errors->has('zip_code') ? 'has-error' : ''}}">
                                                <input type="text" name="zip_code" value="{{$user->zip_code or ''}}" placeholder="Zip / Postal Code *" class="checkout-form-input">
                                            ...
                                            @foreach($countries as $country)
                                                    <option value="{{ $country->id or ''}}" {{ $user ? (($user->country == $country->id) ? 'selected' : '') : '' }}>{{ $country->name }}</option>
                                                @endforeach
                                            </select>
                                            ...
                                            <select name="state" class="checkout-form-input select-state">
                                                ...
                                                @foreach($states as $state)
                                                    <option value="{{ $state->id  or ''}}"  {{ $user ? (($user->state == $state->id) ? 'selected' : '') : '' }}>{{ $state->name }}</option>
                                                @endforeach
                                            </select>
                                            <div class="form-group {{ $errors->has('contact_no') ? 'has-error' : ''}}">
                                                <input type="text" name="contact_no" value="{{$user->contact_no or ''}}" placeholder="Mobile Phone" class="checkout-form-input">
